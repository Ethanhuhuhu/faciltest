...
class Appr(Inc_Learning_Appr):
    ...
    def _get_optimizer(self,lr=0,stage2=False):
        ...
        if stage2:
            ...
            if self.less_forget:
                # Don't update heads when Less-Forgetting constraint is activated (from original code)
                params = self.model.heads[-1].parameters()
            else:
                params = self.heads.parameters()
            return torch.optim.SGD([{"params":params},{"params":self.lws_models.parameters()}], 
            lr, weight_decay=self.wd, momentum=self.momentum)
            # return torch.optim.SGD([{"params":self.model.model.parameters()},{"params":self.lws_models.parameters()}], 
            #     lr, weight_decay=self.wd, momentum=self.momentum)
        else:
            if lr == 0:
                lr=self.lr
    ...
    # add exemplars to train_loader
        if len(self.exemplars_dataset) > 0 and t > 0:
            trn_loader = torch.utils.data.DataLoader(trn_loader.dataset + self.exemplars_dataset,
                                                    ...
                                                    num_workers=trn_loader.num_workers,
                                                    pin_memory=trn_loader.pin_memory)
        # balance_sampler = stage2_utils.ClassAwareSampler(trn_loader.dataset + self.exemplars_dataset)
        # balanced_trn_loader = DataLoader(trn_loader.dataset + self.exemplars_dataset,
        #                         batch_size=trn_loader.batch_size,
        #                         shuffle=False,
        #                         num_workers=trn_loader.num_workers,
        #                         pin_memory=trn_loader.pin_memory,
        ...
        # else:
        # for i in range(len(trn_loader.dataset)):
        #     print('i:'+str(trn_loader.dataset[i]))
        ...
        # # super().train_loop(t, trn_loader, val_loader)
        # if t == 0:
        #     modelstate = torch.load('modeltask0.pt')
    ...
    def post_train_process(self, t, trn_loader):
        ...
        for h in self.ref_model.heads:
            h.train()
        self.ref_model.freeze_backbone()
        ...
        for images, targets in trn_loader:
            ...
            with torch.no_grad():
                for idx in range(len(outputs)):
                    # output.append(self.model.heads[idx](feat.detach()))
                    # print(output[idx])
                    lwsoutputs.append({'wsigma':[],'wosigma':[]})
                    lwsoutputs[idx]['wsigma'] = self.lws_models[idx](outputs[idx]['wsigma'])
                    lwsoutputs[idx]['wosigma'] = self.lws_models[idx](outputs[idx]['wosigma'])
            # outputs = self.lws_models(outputs['wsigma'])
    ...
    def train_epoch_stage2(self , t,train_loader):
        ...
        for i, (images, target) in enumerate(train_loader):
            ...
            for idx in range(len(self.model.heads)):
                ...
                # print(output[idx])
                output[idx]['wsigma'] = self.lws_models[idx](output[idx]['wsigma'])
                output[idx]['wosigma'] = self.lws_models[idx](output[idx]['wosigma'])
    ...
    #         # Eq. 1: regular cross entropy
    #         loss_ce = nn.CrossEntropyLoss()(torch.cat(lwsoutputs, dim=1), targets)
    ...
    def criterion(self, t, outputs, targets, ref_outputs=None, features=None, ref_features=None,stage2 = False,lwsoutputs = []):
        ...
        if ref_outputs is None or ref_features is None or stage2:
            # print('stage2')
            if lwsoutputs!=[]:
                if type(lwsoutputs[0]) == dict:
                    outputs = torch.cat([o['wsigma'] for o in lwsoutputs], dim=1)
                else:
                    outputs = torch.cat(lwsoutputs, dim=1)
            else:
                if type(outputs[0]) == dict:
                    outputs = torch.cat([o['wsigma'] for o in outputs], dim=1)
                else:
            ...
            # Eq. 1: regular cross entropy
            loss_ce = nn.CrossEntropyLoss()(torch.cat([o['wsigma'] for o in lwsoutputs], dim=1), targets)
            # Eq. 9: integrated objective
            loss = loss_dist + loss_ce + loss_mr
        return loss
    ...
    #             # Forward current model
    #             outputs = self.model(images.to(self.device))
    #             loss = torch.nn.CrossEntropyLoss(t, outputs, targets.to(self.device))
    #             hits_taw, hits_tag = self.calculate_metrics(outputs, targets)
    #             # Log
    #             total_loss += loss.item() * len(targets)
    #             total_acc_taw += hits_taw.sum().item()
