    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                if type(outputs[0]) == dict:
                    outputst = torch.cat([o['wsigma'] for o in outputs], dim=1)
                else:
                    outputst = torch.cat(outputs, dim=1)
                loss = torch.nn.CrossEntropyLoss(None)(outputst, targets.to(self.device))
                # loss = torch.nn.CrossEntropyLoss()
                # loss = loss(outputs, targets.to(self.device)) 
