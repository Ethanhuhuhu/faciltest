...
def clsanalyze(clsanalysis):
    ...
    # plt.yticks([500,5,10,100],[r'$500$', r'5',r'$10^1$',r'$10^2$'])
    for tasknum in range(len(clsanalysis)):
        tasks.append('Task'+str(tasknum))
        tasknow = list(clsanalysis[tasknum])
        classindex = np.arange(len(tasknow))+index
        tick = tick+[index + len(tasknow)//2]
        plt.bar(classindex,tasknow,width=0.6)
        index+=len(tasknow)
    plt.title('Conventional CIL',fontsize=18)
    plt.xticks(tick,tasks)
    plt.xlim(0,index)
    setnow.tight_layout()
    setnow.savefig("/home/huyusong/code/FACIL/setnow.jpg",transparent=True,dpi=300)
    tasks=[]
    classall=[]
    index = 1
    tick=[]
    # for tasknum in range(len(testanalysis)):
    #     tasks.append('Task'+str(tasknum+1))
    #     tasknow = list(testanalysis[tasknum])
    #     classindex = np.arange(len(tasknow))+index
    #     tick = tick+[index + len(tasknow)//2]
    #     plt.bar(classindex,tasknow)
    #     index+=len(tasknow)
    # plt.xticks(tick,tasks)
    # plt.xlim(0,index)
    # plt.savefig("/home/huyusong/code/FACIL/testsetnow.jpg")
    # print(clsanalysis)